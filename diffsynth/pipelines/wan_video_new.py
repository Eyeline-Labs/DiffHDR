...
from utils.color_utils import ColourUtils, aces2065_to_srgb_torch
from utils.token_utils import build_mask_latents, mask_latents_to_gate, debug_save_token_gate_pil
...
class BasePipeline(torch.nn.Module):
    ...
    def __init__(
        self,
        device="cuda", torch_dtype=torch.float16, vae_dtype=None,
        ...
        self.vae_dtype = vae_dtype if vae_dtype is not None else torch_dtype
    ...
    def preprocess_image(self, image, torch_dtype=None, device=None, pattern="B C H W", min_value=-1, max_value=1, is_exr=False):
        # Transform a PIL.Image to torch.Tensor
        if not is_exr:
            image = torch.Tensor(np.array(image, dtype=np.float32))
        image = image.to(dtype=torch_dtype or self.vae_dtype, device=device or self.device)
        if is_exr:
    ...
    def preprocess_video(self, video, torch_dtype=None, device=None, pattern="B C T H W", min_value=-1, max_value=1, is_exr=False):
        # Transform a list of PIL.Image to torch.Tensor
        video = [self.preprocess_image(image, torch_dtype=torch_dtype, device=device, min_value=min_value, max_value=max_value, is_exr=is_exr) for image in video]
        video = torch.stack(video, dim=pattern.index("T") // 2)
        return video


    def vae_output_to_image(self, vae_output, pattern="B C H W", min_value=-1, max_value=1, is_hdr=False):
        # Transform a torch.Tensor to PIL.Image
        if pattern != "H W C":
            vae_output = reduce(vae_output, f"{pattern} -> H W C", reduction="mean")
        if is_hdr:
            image = (vae_output / 2 + 0.5)
    ...
    def vae_output_to_video(self, vae_output, pattern="B C T H W", min_value=-1, max_value=1, is_hdr=False):
        # Transform a torch.Tensor to list of PIL.Image
        if pattern != "T H W C":
            vae_output = reduce(vae_output, f"{pattern} -> T H W C", reduction="mean")
        video = [self.vae_output_to_image(image, pattern="H W C", min_value=min_value, max_value=max_value, is_hdr=is_hdr) for image in vae_output]
        return video


    def load_models_to_device(self, model_names=[]):
        if self.vram_management_enabled: # ZM: default: False when training, True when inference
            # offload models
    ...
    def freeze_except(self, model_names):
        for name, model in self.named_children():
            if name in model_names:
                print(f"Set model {name} to trainable.")
                model.train()
...
class WanVideoPipeline(BasePipeline):
    ...
    def __init__(self, device="cuda", torch_dtype=torch.bfloat16, tokenizer_path=None, vae_dtype=None):
        super().__init__(
            device=device, torch_dtype=torch_dtype, vae_dtype=vae_dtype,
    ...
    def enable_vram_management(self, num_persistent_param_in_dit=None, vram_limit=None, vram_buffer=0.5, vae_dtype=None):
        self.vram_management_enabled = True
        ...
        if self.vae is not None:
            ...
            enable_vram_management(
                ...
                module_config = dict(
                    offload_dtype=vae_dtype if vae_dtype is not None else dtype,
                    offload_device="cpu",
                    onload_dtype=vae_dtype if vae_dtype is not None else dtype,
                    onload_device=self.device,
                    computation_dtype=vae_dtype if vae_dtype is not None else self.torch_dtype,
                    computation_device=self.device,
                ),
            )
    ...
    def from_pretrained(
        ...
        vace_use_cfa: bool = False,
        wan_dit_use_cfa: bool = False,
        alpha_init: float = 0.0,
        alpha_over_init: Optional[float] = None,
        alpha_under_init: Optional[float] = None,
    ):
        # Redirect model path
        ...
        # Initialize pipeline
        pipe = WanVideoPipeline(device=device, torch_dtype=torch_dtype, vae_dtype=vae_dtype)
        if use_usp: pipe.initialize_usp()

        # Download and load models
        model_manager = ModelManager()
        for model_config in model_configs:
            model_config.download_if_necessary(use_usp=use_usp, local_model_path=local_model_path)
        ...
        tokenizer_config.download_if_necessary(use_usp=use_usp, local_model_path=local_model_path)
        pipe.prompter.fetch_models(pipe.text_encoder)
        pipe.prompter.fetch_tokenizer(tokenizer_config.path)

        if audio_processor_config is not None:
            audio_processor_config.download_if_necessary(use_usp=use_usp, local_model_path=local_model_path)
            from transformers import Wav2Vec2Processor
            pipe.audio_processor = Wav2Vec2Processor.from_pretrained(audio_processor_config.path)
        # Exposure routing
        def _attach_cfa(v):
            blocks = [b for b in v.modules() if b.__class__.__name__ in ("DiTBlock", "VaceWanAttentionBlock")]
            try:
                p0 = next(v.parameters())
                dev, dt = p0.device, p0.dtype
            except StopIteration:
                dev, dt = device, torch_dtype
            for blk in blocks:
                if hasattr(blk, "use_cfa"):
                    blk.use_cfa = True
                if not hasattr(blk, "alpha_over"):
                    blk.alpha_over = torch.nn.Parameter(torch.zeros((), device=dev, dtype=dt) + alpha_init)
                if not hasattr(blk, "alpha_under"):
                    blk.alpha_under = torch.nn.Parameter(torch.zeros((), device=dev, dtype=dt) + alpha_init)
                if alpha_over_init is not None:
                    blk.alpha_over = torch.nn.Parameter(torch.zeros((), device=dev, dtype=dt) + alpha_over_init)
                if alpha_under_init is not None:
                    blk.alpha_under = torch.nn.Parameter(torch.zeros((), device=dev, dtype=dt) + alpha_under_init)

        if vace_use_cfa:
            if getattr(pipe, "vace", None) is not None:  _attach_cfa(pipe.vace)
            if getattr(pipe, "vace2", None) is not None: _attach_cfa(pipe.vace2)
        if wan_dit_use_cfa:
            if getattr(pipe, "dit", None) is not None:  _attach_cfa(pipe.dit)
            if getattr(pipe, "dit2", None) is not None: _attach_cfa(pipe.dit2)
    ...
    @torch.no_grad()
    def test_encode_decode(self, input_video, tiled, tile_size=(30, 52), tile_stride=(15, 26), is_exr=False, preprocess_video=True):
        self.load_models_to_device(["vae"])
        if preprocess_video:
            input_video = self.preprocess_video(input_video, is_exr=is_exr, torch_dtype=torch.float32)
        print(f"encoder input_video.dtype: {input_video.dtype}")
        print(f'input_video.shape: {input_video.shape}, input_video.min(): {input_video.min()}, input_video.max(): {input_video.max()}')
        input_latents = self.vae.encode(input_video, device=self.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride)
        print(f"encoder output dtype: {input_latents.dtype}")
        print(f"decoder input dtype: {input_latents.dtype}")
        decode_image = self.vae.decode(input_latents, device=self.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride)
        print(f'decode_image.shape: {decode_image[0].shape}, decode_image.min(): {decode_image[0].min()}, decode_image.max(): {decode_image[0].max()}')
        print(f"decoder output dtype: {decode_image.dtype}")
        return self.vae_output_to_video(decode_image, pattern="B C T H W", is_hdr=is_exr), input_latents
    ...
    def __call__(
        ...
        # Optimize mean and std
        optimize_mean_std: Optional[bool] = False,
        mean_delta_path: Optional[str] = None,
        std_delta_path: Optional[str] = None,
        srgb_to_lg=False,
        ...
        inputs_shared = {
            ...
            # Add for VACE input with srgb to acescct
            "srgb_to_lg": srgb_to_lg,
            "binary_in_reactive_mask": binary_in_reactive_mask,
        }
        ...
        for progress_id, timestep in enumerate(progress_bar_cmd(self.scheduler.timesteps)):
            ...
            if cfg_scale != 1.0:
                ...
                print("cfg_scale=1.0, no classifier-free guidance.")
        ...
        if optimize_mean_std:
            self.vae.load_updated_mean_std(mean_delta_path=mean_delta_path, std_delta_path=std_delta_path)
        inputs_shared["latents"] = inputs_shared["latents"].to(dtype=torch.float32)
        video = self.vae.decode(inputs_shared["latents"], device=self.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride)

        video = self.vae_output_to_video(video, is_hdr=is_hdr)
...
class WanVideoUnit_VACEVideoTransform(PipelineUnit):
    def __init__(self):
        super().__init__(
            input_params=("input_video", "use_exposure_augmentation", "srgb_to_lg", "use_exposured_gt", "transform_color_at_forward", "vace_reference_image"),
            onload_model_names=("vae",)
        )
        self.colour_utils = ColourUtils()


    def process(self, pipe: WanVideoPipeline, input_video, use_exposure_augmentation=False, srgb_to_lg=False, use_exposured_gt=False, transform_color_at_forward=False, vace_reference_image=None):
        if not transform_color_at_forward:
            return {}
        if input_video is None:
            # inference, only update vace video
            if srgb_to_lg:
                raise NotImplementedError("srgb_to_lg is not implemented yet.")
            return {}
        if use_exposure_augmentation:
            exposure = np.random.randint(-2, 2)
        else:
            exposure = 0
        vace_video = []

        frame_ori_acescct = torch.stack(input_video)
        frame = self.colour_utils.Acescct_to_ACES2065_1(frame_ori_acescct) # convert to ACES2065-1
        frame_srgb = aces2065_to_srgb_torch(frame, exposure_stops=exposure)
        if use_exposured_gt:
            frame = frame * (2.0 ** exposure) # apply exposure
            input_video = self.colour_utils.ACES2065_1_to_Acescct(frame)
        if srgb_to_lg:
            pass
        frame_srgb = input_video
        vace_video = [Image.fromarray((srgb * 255.0 + 0.5).permute(1, 2, 0).numpy().astype(np.uint8)) for srgb in frame_srgb]
        if pipe.scheduler.training:
            return {"input_video": input_video, "vace_video": vace_video}
        else:
            return {}
...
class WanVideoUnit_InputVideoEmbedder(PipelineUnit):
    def __init__(self):
        super().__init__(
            input_params=("input_video", "noise", "tiled", "tile_size", "tile_stride", "vace_reference_image", "is_exr"),
            onload_model_names=("vae",)
        )

    def process(self, pipe: WanVideoPipeline, input_video, noise, tiled, tile_size, tile_stride, vace_reference_image, is_exr=False):
        if input_video is None:
            return {"latents": noise}
        pipe.load_models_to_device(["vae"])
        input_video = pipe.preprocess_video(input_video, is_exr=is_exr)
        input_latents = pipe.vae.encode(input_video, device=pipe.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride).to(dtype=pipe.torch_dtype, device=pipe.device)
        if vace_reference_image is not None:
            if not isinstance(vace_reference_image, list):
                vace_reference_image = [vace_reference_image]
            vace_reference_image = pipe.preprocess_video(vace_reference_image, is_exr=is_exr)
    ...
    def process(self, pipe: WanVideoPipeline, prompt, positive) -> dict:
        ...
        if isinstance(prompt, list):
            ...
            for i in range(len(prompt)):
                if i > 0 and (prompt[i] == None or prompt[i] == ""):
                    prompt_emb_all.append(None)
                else:
                    prompt_emb_all.append(pipe.prompter.encode_prompt(prompt[i], positive=positive, device=pipe.device))
                    print(prompt[i])
            return {"context": prompt_emb_all[0], "context_over_exposed": prompt_emb_all[1], "context_under_exposed": prompt_emb_all[2]}
        else:
...
class WanVideoUnit_VACE(PipelineUnit):
    def __init__(self):
        super().__init__(
            input_params=("vace_video", "vace_video_mask", "vace_reference_image", "vace_scale", "height", "width",
                          "num_frames", "tiled", "tile_size", "tile_stride", "srgb_to_lg", "binary_in_reactive_mask"),
            onload_model_names=("vae",)
        )
    ...
    def process(
        ...
        if vace_video is not None or vace_video_mask is not None or vace_reference_image is not None:
            ...
            if vace_video is None:
                ...
                vace_video = pipe.preprocess_video(vace_video, is_exr=srgb_to_lg)
            if vace_video_mask is None:
                vace_video_mask = torch.ones_like(vace_video)
                vace_video_mask_over_exposed = None
                vace_video_mask_under_exposed = None
            elif isinstance(vace_video_mask[0], list):
                vace_video_mask_all = [pipe.preprocess_video(m, min_value=0, max_value=1) if m is not None else None for m in vace_video_mask]
                vace_video_mask = vace_video_mask_all[0]
                vace_video_mask_over_exposed = vace_video_mask_all[1]
                vace_video_mask_under_exposed = vace_video_mask_all[2]
                if vace_video_mask_over_exposed is not None and vace_video_mask_over_exposed.sum() == 0:
                    vace_video_mask_over_exposed = None
                if vace_video_mask_under_exposed is not None and vace_video_mask_under_exposed.sum() == 0:
                    vace_video_mask_under_exposed = None
            else:
            ...
            if not binary_in_reactive_mask:
                # soft
                inactive = vace_video * (1 - vace_video_mask) + 0 * vace_video_mask
                reactive = vace_video * vace_video_mask + 0 * (1 - vace_video_mask)
            else:
                # binary
                inactive = vace_video * (1 - (vace_video_mask > binary_threshold).to(dtype=vace_video.dtype)) + 0 * vace_video_mask
                reactive = vace_video * (vace_video_mask > binary_threshold).to(dtype=vace_video.dtype) + 0 * (1 - vace_video_mask)
            ...
            vace_mask_latents = torch.nn.functional.interpolate(vace_mask_latents, size=((vace_mask_latents.shape[2] + 3) // 4, vace_mask_latents.shape[3], vace_mask_latents.shape[4]), mode='nearest-exact').to(dtype=pipe.torch_dtype)

            if vace_video_mask_over_exposed is not None:
                over_lat = build_mask_latents(vace_video_mask_over_exposed).to(dtype=pipe.torch_dtype)
                if vace_reference_image is not None:
                    over_lat = torch.concat((torch.zeros_like(over_lat[:, :, :1]), over_lat), dim=2)
                token_mask_over = mask_latents_to_gate(over_lat)
                token_mask_over = (token_mask_over > 0.5).to(token_mask_over.dtype)

            if vace_video_mask_under_exposed is not None:
                under_lat = build_mask_latents(vace_video_mask_under_exposed).to(dtype=pipe.torch_dtype)
                if vace_reference_image is not None:
                    under_lat = torch.concat((torch.zeros_like(under_lat[:, :, :1]), under_lat), dim=2)
                token_mask_under = mask_latents_to_gate(under_lat)
                token_mask_under = (token_mask_under > 0.5).to(token_mask_under.dtype)

            if vace_reference_image is None:
                pass
            else:
                vace_reference_image = pipe.preprocess_video([vace_reference_image], is_exr=srgb_to_lg)
                vace_reference_latents = pipe.vae.encode(vace_reference_image, device=pipe.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride).to(dtype=pipe.torch_dtype, device=pipe.device)
                vace_reference_latents = torch.concat((vace_reference_latents, torch.zeros_like(vace_reference_latents)), dim=1)
                vace_video_latents = torch.concat((vace_reference_latents, vace_video_latents), dim=2)
                vace_mask_latents = torch.concat((torch.zeros_like(vace_mask_latents[:, :, :1]), vace_mask_latents), dim=2)
            vace_context = torch.concat((vace_video_latents, vace_mask_latents), dim=1)
            return {"vace_context": vace_context, "vace_scale": vace_scale, "token_mask_over": token_mask_over, "token_mask_under": token_mask_under}
        else:
            return {"vace_context": None, "vace_scale": vace_scale, "token_mask_over": None, "token_mask_under": None}
...
def model_fn_wan_video(
    ...
    **kwargs,
    ...
    if context_over_exposed is not None:
        context_over_exposed = dit.text_embedding(context_over_exposed)
    if context_under_exposed is not None:
        context_under_exposed = dit.text_embedding(context_under_exposed)
            ...
            if use_gradient_checkpointing_offload:
                with torch.autograd.graph.save_on_cpu():
                    x = torch.utils.checkpoint.checkpoint(
                        ...
                        x, context, t_mod, freqs, None, context_over_exposed, context_under_exposed, token_mask_over, token_mask_under,
                        use_reentrant=False,
                    )
                ...
                x = block(x, context, t_mod, freqs, None, context_over_exposed, context_under_exposed, token_mask_over, token_mask_under)
